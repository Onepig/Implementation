...
class SVM:
    def __init__(self,dataMatIn, classLabels, C=2, toler=0.001, iterMax=20, KernelType='linear', gama=0.0000001):  # Initialize the structure with the parameters
        ...
        for i in range(self.m):
            self.K[:,i] = self.__kernelTrans(self.X, self.X[i,:],self.KernelType, self.gama)
    ...
    def __kernelTrans(self,X, A, kernelType, parameter): #calc the kernel or transform data to a higher dimensional space
        ...
        elif kernelType=='rbf': # rbf kernel, radial basis function
            for j in range(m):
                deltaRow = X[j,:] - A
    ...
    def predict(self, testData,testLabel=None):
        ...
        m,n = shape(datMat)
        svInd=nonzero(self.alphas.A>0)[0]
        sVs=datMat[svInd]
        labelSV = labelMat[svInd];
        errorCount=0
        for i in range(m):
            kernelEval = self.__kernelTrans(sVs,datMat[i,:],self.KernelType,self.gama)
            predict=(kernelEval.T * multiply(labelSV,self.alphas[svInd]) + self.b)
            if sign(predict[0,0])!=sign(labelMat[i,0]):
                errorCount += 1
    ...
    def showFig(self):
        ...
        ax = plt.subplot(111)
        color=['r','g','b']
        c=array(ones((self.m,1)))
        X=array(zeros((self.m,1)))
        Y=array(zeros((self.m,1)))
        cp=None
        for i in range(self.m):
            X[i,0]=self.X[i,0]
            Y[i,0]=self.X[i,1]
            if self.labelMat[i,0]<0:
                c[i,0]=0
        ax.scatter(X,Y,s=50,c=15*c)
        plt.show()
...
if __name__=="__main__":
    import getopt
    import sys
    trainData,trainLabel=loadDataSet(r"E:\Study\Implementation\testSetRBF2.txt",False)
